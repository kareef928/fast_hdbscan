...
def hdbscan_lambda_value(distance):
    if distance > 0.0:
        return 1.0 / distance
    else:
        return np.inf
...
@numba.njit(inline="always", fastmath=True)
def plscan_lambda_value(distance):
...
def condense_tree(
    ...
    ignore = np.zeros(
        root + 1, dtype=np.bool_
    )  # 'bool' is no longer an attribute of 'numpy'
    ...
    for node in node_list:
        ...
        right = np.int64(r)
        lambda_value = (
            hdbscan_lambda_value(d)
            if lambda_method == "hdbscan"
            else plscan_lambda_value(d)
        )
...
def eom_recursion_bcubed(
    node,
    cluster_tree,
    stability_node_scores,
    bcubed_node_scores,
    selected_clusters,
    unselected_nodes,
):

    current_score_stability_bcubed = np.array(
        [stability_node_scores[node], bcubed_node_scores[node]], dtype=np.float32
    )
    ...
    for child_node in children:
        child_score_total_stability_bcubed += eom_recursion_bcubed(
            child_node,
            cluster_tree,
            stability_node_scores,
            bcubed_node_scores,
    ...
    if child_score_total_stability_bcubed[1] > current_score_stability_bcubed[1]:
        ...
        unselect_below_node_bcubed(
            node, cluster_tree, selected_clusters, unselected_nodes
        )
    ...
    for cluster, cluster_labeled_points in cluster_elements.items():
        ...
        if len(cluster_labeled_points) > 0:
            ...
            for label, num_points in cluster_labeled_points_dict.items():
                ...
                precision_point = (
                    num_points / num_labeled_in_node
                ) / total_num_of_labeled_points
                recall_point = (
                    num_points / total_num_of_class_label
                ) / total_num_of_labeled_points

                # Bcubed F-measure
                bcubed[cluster] += num_points * (
                    2.0 / (1.0 / precision_point + 1.0 / recall_point)
                )
    return bcubed
...
def unselect_below_node_bcubed(node, cluster_tree, selected_clusters, unselected_nodes):
    ...
    for child in cluster_tree.child[cluster_tree.parent == node]:
        if not unselected_nodes[child]:
            unselect_below_node_bcubed(
                child, cluster_tree, selected_clusters, unselected_nodes
            )
...
def extract_clusters_bcubed(
    ...
    if allow_virtual_nodes:
        ...
        bcubed_node_scores = score_condensed_tree_nodes_bcubed(
            cluster_elements, label_indices
        )
    ...
    if allow_single_cluster:
        ...
        root_children = cluster_tree_bcubed.child[
            cluster_tree_bcubed.parent == cluster_tree_root
        ]
    ...
    return np.asarray(
        [
            node
            for node, selected in selected_clusters.items()
            if (selected and (node not in virtual_nodes))
        ]
    )
...
def eom_recursion(
    ...
    )  # floating point errors can make score negative!
    ...
    if len(cluster_tree.parent) > 0:
        node_sizes = {
            ...
            for node, size in zip(
                cluster_tree.child, cluster_tree.child_size.astype(np.float32)
            )
        }
        node_sizes[cluster_tree.parent.min()] = np.float32(
            cluster_tree.parent.min() - 1
        )
    else:
